                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <ul class="list-group list-group-flush">
                                    ...
                                    <li class="list-group-item px-0"><strong>Order Date:</strong>
                                        {{ $order->order_date ? \Carbon\Carbon::parse($order->order_date)->format('d F Y') : 'N/A' }}
                                    </li>
                                    <li class="list-group-item px-0"><strong>Due Date:</strong>
                                        {{ $order->due_date ? \Carbon\Carbon::parse($order->due_date)->format('d F Y') : 'N/A' }}
                                    </li>
                                ...
                                <table class="table table-striped table-hover">
                                    ...
                                    <tbody>
                                        @foreach ($order->items as $item)
                                            <tr>
                                                <td>
                                                    <strong>{{ $item->name ?? ($item->serviceItem ? $item->serviceItem->name : 'N/A') }}</strong>
                                                </td>
                                                <td>
                                                    {{ $item->serviceItem && $item->serviceItem->category ? $item->serviceItem->category->name : 'N/A' }}
                                                </td>
                                                ...
                                                <td class="text-end">
                                                    {{ $user && $user->appSetting && $user->appSetting->business_currency ? $user->appSetting->business_currency : 'NGN' }}{{ number_format($item->price, 2) }}
                                                </td>
                                                <td class="text-end">
                                                    <strong>{{ $user && $user->appSetting && $user->appSetting->business_currency ? $user->appSetting->business_currency : 'NGN' }}{{ number_format($item->subtotal, 2) }}</strong>
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                    ...
                    <div class="card-body">
                                ...
                                <div class="card bg-light">
                                    <div class="card-body text-center">
                                        ...
                                        <h4 class="text-success">
                                            {{ $user && $user->appSetting && $user->appSetting->business_currency ? $user->appSetting->business_currency : 'NGN' }}{{ number_format($order->payments ? $order->payments->sum('amount') : 0, 2) }}
                                        </h4>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="card bg-light">
                                    <div class="card-body text-center">
                                        ...
                                        <h4 class="text-warning">
                                            {{ $user && $user->appSetting && $user->appSetting->business_currency ? $user->appSetting->business_currency : 'NGN' }}{{ number_format($order->getOutstandingBalance(), 2) }}
                                        </h4>
                                    </div>
                                </div>
                            </div>
                                ...
                                <table class="table table-striped table-hover">
                                    ...
                                    <tbody>
                                        @foreach ($order->payments as $payment)
                                            <tr>
                                                <td>
                                                    <span class="badge bg-gradient-info text-white text-capitalize">
                                                        {{ $payment->mode_of_payment }}
                                                    </span>
                                                </td>
                                                <td>
                                                    <strong>{{ $user && $user->appSetting && $user->appSetting->business_currency ? $user->appSetting->business_currency : 'NGN' }}{{ number_format($payment->amount, 2) }}</strong>
                                                </td>
                                                <td>
                                                    {{ $payment->notes ?? '-' }}
                                                </td>
                                                <td>
                                                    {{ $payment->created_at ? $payment->created_at->format('d F Y, h:i A') : 'N/A' }}
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                        ...
                        @else
                            <div class="alert alert-warning mb-0">
                                <i class="material-symbols-rounded align-middle me-2">warning</i>
                ...
                <div class="card shadow-xl pb-0 p-3">
                    <div class="row gx-4 mb-2">
                        ...
                        <div class="col-auto my-auto">
                            ...
                            <ul class="list-group">
                                <li class="list-group-item border-0 ps-0 pt-0 text-sm"><strong class="text-dark">Business
                                        Name:</strong> &nbsp;
